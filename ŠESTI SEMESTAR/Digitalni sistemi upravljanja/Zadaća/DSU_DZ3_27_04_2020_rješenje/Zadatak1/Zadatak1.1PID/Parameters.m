...
% Ziegler-Nicholas metoda
 ...
 Ti = 2*tau;         % Integraciona vrem. konst.
 Td = 0.5*tau;       % Derivaciona vrem. konst.

%Takahashi metoda
% Ki = 0.6*T0*T/K0/(tau + 0.5*T)^2;      % Pojacanje integracionog dejstva
% Kp = 1.2*T0/K0/(tau + T) - 0.5*Ki/K0;  % Pojacanje proporcionalnog dejstva
% Kd = 0.5*T0/K0/T;                      % Pijacanje prop. dejstva
% Ti = Kp*T/Ki;                          % Integraciona vrem. konst.
% Td = T*Kd/Kp;                          % Derivaciona vrem. konst
